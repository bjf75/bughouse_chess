...
BUGHOUSE_BOARD_IDS = [A, B] = ['A', 'B']

def opposite_bughouse_board_id(board_id: BUGHOUSE_BOARD_IDS) -> BUGHOUSE_BOARD_IDS:
    if board_id == A:
        return B
    return A
...
STARTING_BFEN = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR[] w KQkq - 0 1 | rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR[] w KQkq - 0 1"
"""The BFEN for the standard bughouse chess starting position."""

STARTING_BOARD_FEN = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR"
"""The board part of the FEN for the standard chess starting position."""

STARTING_BOARD_BFEN = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR[]|rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR[]"
"""The board part (and pockets) of the BFEN for the standard bughouse chess starting position."""
...
def popcount(bb: Bitboard, *, _bin: Callable[[int], str] = bin) -> int:
    if _bin != bin:
        return _old_popcount(bb)
    else:
        return _new_popcount(bb)

def new_popcount(bb: Bitboard) -> int:
    """
    Explicitly uses the new popcount.
    """
    return _new_popcount(bb)

def _new_popcount(bb: Bitboard) -> int:
    """
    This implements the map to number of bits (which is constant time) implementation
    for counting the number of bits that are 1 in an integer

    Designed according to the fourth method at https://www.geeksforgeeks.org/count-set-bits-in-an-integer/
    """
    ...
    while bb_copy != 0:
        nibble = bb_copy & 0xf
        total += num_to_bits[nibble]
        bb_copy = bb_copy >> 4
    return total

def _old_popcount(bb: Bitboard, *, _bin: Callable[[int], str] = bin) -> int:
    """
    This is the old popcount implementation from python-chess; I'm pretty sure this
    is not a fast implementation, which is why I created the new implementation.
    """
...
class Move:
    ...
    def __init__(self, from_square: Square, to_square: Square, promotion: Optional[PieceType] = None, drop: Optional[PieceType] = None, board: Optional[BoardT] = None, capture: Optional[bool] = False) -> None:
...
class BaseBoard:
    ...
    def _repr_svg_(self) -> str:
        from . import svg
        return svg.board(board=self, size=400)
...
class _BoardState(Generic[BoardT]):
    ...
    def restore(self, board: BoardT) -> None:
        ...
        # I genuinely cannot explain what is happening right now.
        # in the next line, the number of knights in the opposite board's pocket
        # increases by 2; in the following line, it increases by another two;
        # this SHOULD NOT BE POSSIBLE. I'm so lost as to what to do now.
        # I'm going to try commenting them out to see what happens.
        # Okay, so, this is a verrrry strange bug that occurs when running this
        # using the VS Code debugger, but not any other time
...
class Board(BaseBoard):
    ...
    def __init__(self: BoardT, fen: Optional[str] = STARTING_FEN, *, chess960: bool = False) -> None:
        ...
        self.move_stack = list()  # type: List[Move]
        self._stack = list()  # type: List[_BoardState[BoardT]]
    ...
    def _push_no_capture(self):
        """
        Only classes that need it should implement
        :return:
        """
        pass

    def push(self: BoardT, move: Move, pocket_pushing: bool=False) -> None:
        """
        ...
        if not move:
            ...
            self._push_no_capture(pocket_pushing)
            return
        ...
        if not castling:
            ...
            if captured_piece_type:
                self._push_capture(move, capture_square, captured_piece_type, was_promoted, pocket_pushing)
            else:
                self._push_no_capture(pocket_pushing)
        else:
    ...
    def _parse_epd_ops(self: BoardT, operation_part: str, make_board: Callable[list(), BoardT]) -> Dict[str, Union[None, str, int, float, Move, List[Move]]]:
        operations = dict()  # type: Dict[str, Union[None, str, int, float, Move, List[Move]]]
        state = "opcode"
        opcode = ""
        operand = ""
        position = None
...
class SquareSet:
    ...
    def _repr_svg_(self) -> str:
        from . import svg
        return svg.board(squares=self, size=400)
